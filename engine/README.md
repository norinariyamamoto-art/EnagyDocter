# Energy Doctor 診断Engine（Task 1 実装）
...
engine/
  energy_doctor_engine/
    excel_compat.py     Excelの計算クセ（空欄比較、AVERAGE、ROUND等）を再現する共通関数
    ...
    pipeline.py             上記を正しい依存順序で実行する end-to-end 関数
  tests/
    fixtures.py             TC-A/B/Cの入力データ（出典はfixtures.py内のdocstring参照）
...
19件のテストがすべてPASSすることを確認済み（2026-09-01時点）。
...
| `top5_final.py` | `TOP5_Final` | TOP-R02重複統合・TOP-R03分野上限・最終順位（候補のみのランキング） |
